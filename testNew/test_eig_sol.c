...
int TestEigenSolver(void *A, void *B, int argc, char *argv[], struct OPS_ *ops) 
{
...
#else
	//int    nevMax = 1, multiMax = 1, block_size = 1;
	int    nevMax = 75, multiMax = 75, block_size = 15;
	//int    nevMax = 600, multiMax = 60, block_size = 120;
	...
	//	ops->MatView(B,ops);
	//}
	ops->MultiVecCreateByMat(&evec,sizeX,A,ops);
	...
	EigenSolverCreateWorkspace_GCG(sizeX,block_size,A,gcg_mv_ws,&dbl_ws,&int_ws,ops);	
#endif	
	//nevInit = 25;
	ops->Printf("===============================================\n");
...
#else
	EigenSolverDestroyWorkspace_GCG(sizeX,block_size,A,gcg_mv_ws,&dbl_ws,&int_ws,ops);		
#endif
